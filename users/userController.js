...
const {AdminAddItemsModel,CartModel, ordersModel}=require('../Admin/adminSchema');
...
class UserController{
    ...
    async loginUser(req,res){
        try {
            ...
            const token = jwt.sign({id: user}, process.env.USER_SECRET_KEY, {expiresIn: '1h'});
    ...
    async cartData(req,res){
        try {
            ...
            const cartItem = new CartModel({userId,itemId: id,quantity: quantity});
             await cartItem.save();
            return res.status(200).json({ message: 'Items Added successfully',data:cartItem });
    ...
    async getCartData(req,res){
        try {
            ...
            return res.status(200).json({ message: 'Items fetched successfully',data:itemsdatas });
        } catch (error) {
            console.log(error);
    ...
    try {
        ...
        await CartModel.findByIdAndDelete(id);
        return res.status(201).json({ message: 'Order placed successfully', order: orderData });
        } catch (error) {
