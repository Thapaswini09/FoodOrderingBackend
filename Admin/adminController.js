...
class AdminController {
  ...
  async adminLogin(req, res) {
    try {
      ...
      const token = jwt.sign({ id: admin }, process.env.ADMIN_SECRET_KEY, {
        expiresIn: "1h",
      });
      ...
      return res
        .status(200)
        .json({ message: "Login successfully", data: admin, token: token });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
  ...
  async getAdminAddItems(req, res) {
    try {
      const adminItems = await AdminAddItemsModel.find({
        adminId: req.admin.id._id,
      });
      return res
        .status(200)
        .json({ message: "Items fetched successfully", data: adminItems });
    } catch (error) {
      return res.status(500).json({ message: "Internal server error" });
    }
  }
